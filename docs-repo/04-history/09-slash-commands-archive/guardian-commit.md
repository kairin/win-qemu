...
If files match:
  ...
  scripts/*.sh → feat (automation scripts)
  configs/*.xml → config (VM configurations)
  research/*.md → docs (research documentation)
  outlook-linux-guide/*.md → docs (implementation guides)
  AGENTS.md, README.md → docs (core documentation)
  .gitignore, .env.example → config (configuration)
```
...
- From directory: configs/ → scope: configs
- From directory: research/ → scope: research
...
- ❌ Does NOT run health checks (use `/guardian-health`)
...
- ✅ Automatic commit type detection (feat, fix, docs, config)
...
- ✅ QEMU/KVM-specific commit categories (config for VM XML, etc.)
